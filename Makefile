...
localnet-start: localnet-stop build-docker-localnode
	@if ! [ -f build_src/node0/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build_src:/tendermint:Z tendermint/localnode_src testnet --config config-template.toml --o . --starting-ip-address 192.167.10.2; fi
